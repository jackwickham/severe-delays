...
impl AbstractStore for SqliteStore {
    async fn get_status_history<'a>(&'a self, start_time: OffsetDateTime, end_time: OffsetDateTime) -> HashMap<String, Vec<LineHistoryEntry>> {
        sqlx::query_as::<_, SqliteHistoryEntry>("SELECT * FROM history WHERE start_time <= ? AND (end_time IS NULL OR end_time >= ?)")
            ...
            .into_iter()
            .map(|row| {
                (row.line, LineHistoryEntry {
                    start_time: OffsetDateTime::from_unix_timestamp(row.start_time).unwrap(),
                    end_time: row.end_time.map(|t| OffsetDateTime::from_unix_timestamp(t).unwrap()),
                    data: serde_json::from_slice(&row.data).unwrap(),
                })
            })
            .fold(HashMap::<String, Vec<LineHistoryEntry>>::new(), |mut acc, (line, entry)| {
    ...
    async fn set_status(&self, status_by_line: HashMap<String, Value>) {
        ...
        let existing = sqlx::query_as::<_, SqliteHistoryEntry>("SELECT * FROM history WHERE end_time IS NULL")
            .fetch_all(&self.pool)
            .await
            .unwrap()
            .into_iter()
        ...
        for (line, status) in status_by_line {
            if let Some(existing) = existing.get(&line) {
                ...
                log::info!("Found existing row, but it didn't match - updating: {:?}", line);
            ...
            sqlx::query("INSERT INTO history (line, start_time, end_time, data) VALUES (?, ?, NULL, ?)")
                .bind(&line)
