...
export const Line: Component<LineProps> = (props: LineProps) => {
  const statusHistoryInRange = createMemo<RenderedHistoryEntry[]>(() => {
    ...
    for (let i = 0; i < props.statusHistory.length; i++) {
      ...
      const endTime = nextEntryStartTime ? Math.min(nextEntryStartTime.getTime(), props.displayRange.end.getTime()) : props.displayRange.end.getTime();
  ...
  const basisMultiplier = () => 10000 / (props.displayRange.end.getTime() - props.displayRange.start.getTime());
  ...
  const performanceStats = createMemo(() => {
    ...
    for (const statusHistory of statusHistoryInRange()) {
      if (statusHistory.status.state !== State.OTHER) {
        durations[statusHistory.status.state] = statusHistory.displayEndTime - statusHistory.displayStartTime;
    ...
    <div>
      <h2 class="text-2xl mb-3">
        <Show when={props.colour}>
          <div class="inline-block w-4 h-4 rounded-full mr-2" style={`background-color: rgb(${props.colour!.r}, ${props.colour!.g}, ${props.colour!.b})`}></div>
        </Show>
        {props.name}
      </h2>
      <div class="flex flex-row flex-wrap justify-after text-sm text-slate-500 space-x-3 mb-2">
        <p>Performance: </p>
        <For each={performanceStats()}>{(perf) => <p><span class={`w-2 h-2 rounded-full inline-block ${getStatusColour(perf.state)}`}></span> {perf.state}: {perf.performancePercentage.toFixed(1)}%</p>}</For>
      </div>
      <div class="flex flex-row items-center bg-slate-300 h-2 rounded">
        <For each={statusHistoryInRange()}>{(entry, i) => {
