...
export const LineHistory: Component = () => {
  const statusHistory = [
    ...
    },
    {
      ...
      status: {
        ...
        reason: "METROPOLITAN LINE: Saturday 7 and Sunday 8 October, no service between Wembley Park and Amersham / Chesham / Uxbridge / Watford. Chiltern Railways services will not run between Marylebone and Amersham. Replacement buses operate, but will not serve Preston Road, Northwick Park or stations between Harrow-on-the-Hill and Uxbridge. Please use local London Buses services.",
      }
    },
    {
  ...
  return (<div class="space-y-10">
    <For each={lines}>{line => <Line name={line.name} statusHistory={line.statusHistory} colour={line.colour} displayRange={displayRange} />}</For>
  </div>);
};
